...
class KnowSayinCloudHandler(BaseHTTPRequestHandler):
            ...
            self._send_json(
                {
                    ...
                    "anonymousDailyLimit": settings.anonymous_daily_limit,
    ...
    def _handle_session(self) -> None:
        ...
        with usage_lock:
            ...
            sessions[token_hash] = {"createdAt": _now_iso()}
            _write_data_file(payload)
        self._send_json(
            {
                "token": token,
                "dailyLimit": settings.anonymous_daily_limit,
                "remaining": settings.anonymous_daily_limit,
                "maxChars": settings.max_chars,
            }
        )
    ...
    def _handle_clean(self) -> None:
        ...
        if access.remaining <= 0:
            self._send_json({"error": "DAILY_LIMIT", "message": "今日免费额度已用完。"}, HTTPStatus.TOO_MANY_REQUESTS)
            return
        ...
        usage = _increment_usage(access.token_hash, access.ip_hash, len(text))
        self._send_json(
            {
                "result": result,
                "remaining": max(settings.anonymous_daily_limit - usage["count"], 0),
                "dailyLimit": settings.anonymous_daily_limit,
                "maxChars": settings.max_chars,
            }
        )
    ...
    def _resolve_access(self) -> AccessContext | tuple[dict[str, Any], HTTPStatus]:
        ...
        with usage_lock:
            payload = _read_data_file()
            if token_hash not in payload.get("sessions", {}):
                return {"error": "BAD_TOKEN", "message": "session 已失效，请重新连接。"}, HTTPStatus.UNAUTHORIZED
            token_usage = _token_usage(payload, token_hash)
            ip_usage = _ip_usage(payload, ip_hash)

        return AccessContext(
            token_hash=token_hash,
            ip_hash=ip_hash,
            remaining=max(settings.anonymous_daily_limit - token_usage["count"], 0),
...
def _load_settings() -> CloudSettings:
    return CloudSettings(
        anonymous_daily_limit=_env_int("KNOWSAYIN_API_ANON_DAILY_LIMIT", 20),
...
def _read_data_file() -> dict[str, Any]:
    ...
    if payload.get("date") != _today_key():
        ...
        return {**_empty_data(), "sessions": sessions}
...
def _token_usage(payload: dict[str, Any], token_hash: str) -> dict[str, int]:
    current_minute = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M")
    usage = payload.get("tokens", {}).get(token_hash) or {}
    return {
...
def _increment_usage(token_hash: str, ip_hash: str, chars: int) -> dict[str, int]:
    with usage_lock:
        payload = _read_data_file()
        current_minute = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M")
        current_hour = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H")
        ...
        _write_data_file(payload)
        return {"count": int(token_usage["count"]), "chars": int(token_usage["chars"])}
